...
_DT = 0.008              # 125 Hz servo loop 
_AMPLITUDE = 2.0         # ±2 deg base oscillation
_PERIOD = 4.0            # seconds per full oscillation
_N_CYCLES = 2
...
def main(robot_ip: str):
    ...
    try:
        ...
        logger.info(
            f"Oscillating j0 by ±{_AMPLITUDE} deg around {center[0]:.3f} deg "
            f"({_N_CYCLES} cycles, {_PERIOD}s each)"
        )

        duration = _PERIOD * _N_CYCLES
        t0 = time.monotonic()
        while True:
            t = time.monotonic() - t0
            if t >= duration:
            ...
            target = list(center)
            target[0] = center[0] + _AMPLITUDE * math.sin(theta)
