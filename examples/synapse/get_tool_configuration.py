"""
Get Tool configuration examples for the Synapse SDK.
...
def get_payload_example(ip: str):
    ...
    # Read configured payload mass and report the result
    try:
...
def get_payload_cog_example(ip: str):
    ...
    # Read configured payload CoG and report the result
    try:
...
def get_payload_inertia_example(ip: str):
    """Read configured payload inertia tensor [Ixx, Iyy, Izz, Ixy, Ixz, Iyz] [kg·m²]."""

    # Create and connect to the robot
    robot = universal_robots.UniversalRobotsUR10E()
    robot.connect(ip=ip)

    # Read configured payload inertia tensor and report the result
    try:
