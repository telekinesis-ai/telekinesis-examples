...
def run_examples():
    ...
    examples = sorted([f for f in examples_dir.glob("*.py") if not f.name.startswith("_") and f.name != "run_vitreous_examples.py"])
    ...
    for name, fetch, algorithm, visualize, total, status in sorted(timings, key=lambda t: t[4], reverse=True):
        tot_fetch += fetch
        tot_algo += algorithm
        tot_viz += visualize
        tot_total += total
    ...
    logger.info(rule)
    total_row = f"{tot_fetch:>7.2f} | {tot_algo:>7.2f} | {tot_viz:>7.2f} | {tot_total:>7.2f}"
