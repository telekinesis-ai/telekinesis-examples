...
# Drop pose
DROP_POSE = [-0.623563, -0.001, 0.27432, -180.0, 0.0, -180.0]

# Pick parameters
APPROACH_HEIGHT = 0.15  # meters above object before descent
GRASP_Z = 0.01          # final pick z (meters, world frame); tune to table height
...
# Detection
QWEN_PROMPT = (
    "Detect pickable silver circles. "
)
...
def deproject_pixel_to_world(pixel_xy, depth, intrinsics, world_T_camera):
    ...
    x0 = max(cx - 3, 0); x1 = min(cx + 4, W)
    y0 = max(cy - 3, 0); y1 = min(cy + 4, H)
    ...
    return np.array(world_T_pt_mat.to_numpy()) if hasattr(world_T_pt_mat, "to_numpy") else np.array(world_T_pt_mat)
        ...
        for j, it in enumerate(items):
            ...
            if isinstance(seg, dict):
                ...
                if "mask" in seg:
                    ...
                    logger.warning(f"    SAM item {j}: RLE segmentation, no decoder available, skipping")
                    mask = None
                ...
                if arr.ndim == 2 and arr.shape == (H, W):
                    ...
                    try:
                        ...
                        polys = seg if (isinstance(seg, list) and len(seg) > 0 and isinstance(seg[0], (list, np.ndarray))) else [seg]
                        for poly in polys:
                            pts = np.asarray(poly, dtype=np.float32).reshape(-1, 2).astype(np.int32)
                            cv2.fillPoly(canvas, [pts], 255)
                        mask = canvas.astype(bool)
                        logger.info(f"    SAM item {j}: decoded polygon mask, pixel count = {int(mask.sum())}")
                    except Exception as poly_e:
                        logger.warning(f"    SAM item {j}: unexpected segmentation shape {arr.shape}, could not decode as polygon: {poly_e}")
                        mask = None
            if mask is None:
                ...
                if px < MIN_SAM_MASK_PX:
                    logger.warning(f"    SAM item {j}: mask too small ({px} px < {MIN_SAM_MASK_PX}), rejecting")
                    mask = None
                else:
...
def compute_yaw_world_from_axis(centroid_uv, principal_axis_uv, depth, intrinsics, world_T_camera, k_pixels):
    ...
    try:
        ...
        for k, centroid in enumerate(centroids):
            ...
            if cluster_yaw is None:
                ...
                logger.info(f"  cluster {k}: no yaw available, using default PICK_ORIENTATION={pick_orientation}")
            else:
