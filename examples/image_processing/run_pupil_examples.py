...
def run_examples():
    """Discover and run all pupil examples."""
    examples_dir = pathlib.Path(__file__).parent
    examples = sorted([f for f in examples_dir.glob("*.py") if not f.name.startswith("_") and f.name != "run_pupil_examples.py"])

    if not examples:
        logger.error("No examples found")
        return 1

    logger.info(f"Running {len(examples)} pupil example(s)...")
    logger.info("=" * 60)
